...
class AutoPartBrandImporter extends Importer
{
    ...
    {
        return [
            ImportColumn::make('name')->label('Brand Name')->rules(['required', 'string', 'max:255']),
    ...
    public function resolveRecord(): ?AutoPartBrand
    {
        return AutoPartBrand::firstOrNew([
            'name' => $this->data['name'],
        ]);
    }
    ...
    public function afterSave(): void
    {
        if (!empty($this->data['logo_url']) && filter_var($this->data['logo_url'], FILTER_VALIDATE_URL)) {
            try {
                ...
                \Log::error("Failed to import autopart brand logo [{$this->record->name}]: " . $e->getMessage());
            }
        }
    }
    ...
    {
        $body = 'AutoPart Brands imported successfully: ' . number_format($import->successful_rows) . ' rows.';
