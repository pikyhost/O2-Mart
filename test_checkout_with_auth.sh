...
echo "=== Testing Checkout Flow with Authentication ==="
...
PRODUCT_RESPONSE=$(curl -s "$BASE_URL/api/batteries" \
  -H "Authorization: Bearer $TOKEN")
echo "Product response (first 200 chars): ${PRODUCT_RESPONSE:0:200}..."

PRODUCT_ID=$(echo $PRODUCT_RESPONSE | grep -o '"id":[0-9]*' | head -1 | cut -d':' -f2)
echo "Using product ID: $PRODUCT_ID"

if [ -z "$PRODUCT_ID" ]; then
  echo "ERROR: Could not get product ID"
  exit 1
fi
